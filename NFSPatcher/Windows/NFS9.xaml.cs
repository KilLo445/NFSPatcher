...
{
    public partial class NFS9 : Window
    {
        // Strings
        string gameName = "Need for Speed: Most Wanted";
        string gameNameS = "NFS: Most Wanted";
        ...
        string patchName = "nfsmwpatch1.3";
        string patchNameZip = "nfsmwpatch1.3.zip";
        string patchNameExe = "nfsmwpatch1.3.exe";
        ...
        public NFS9()
        {
            ...
            tempPath = Path.Combine(Path.GetTempPath(), "NFSPatcher", "MW");

            patchZip = Path.Combine(tempPath, patchNameZip);
            patchExe = Path.Combine(tempPath, patchNameExe);
            wfpZip = Path.Combine(tempPath, "NFSMostWanted.WidescreenFix.zip");
        ...
        private void MainPatch()
        {
            ...
            try
            {
                StatusText.Text = "Downloading v1.3 Patch... (1 of 3)";
                pb.IsIndeterminate = false;
        ...
        private async Task InstallPatch()
        {
            ...
            try
            {
                ...
                StatusText.Text = $"Extracting NFSMostWanted.WidescreenFix.zip..."; await Task.Run(() => ZipFile.ExtractToDirectory(wfpZip, wfpPath));
                StatusText.Text = $"Deleting NFSMostWanted.WidescreenFix.zip..."; await Task.Run(() => File.Delete(wfpZip));
                await Task.Delay(500);
                ...
                if (wfpCFG == MessageBoxResult.Yes)
                {
                    ...
                    string wfpCFGPath = Path.Combine(installPath, "scripts", "NFSMostWanted.WidescreenFix.ini");
