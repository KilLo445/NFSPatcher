...
namespace NFSPatcher.Windows
{
    public partial class NFS7 : Window
    {
        ...
        public NFS7()
        {
            ...
            patchZip = Path.Combine(tempPath, "US_PATCH_4.zip");
            patchExe = Path.Combine(tempPath, "US_PATCH_4.exe");
        ...
        private async Task InstallPatch()
        {
            ...
            try
            {
                ...
                StatusText.Text = $"Extracting US_PATCH_4.zip..."; await Task.Run(() => ZipFile.ExtractToDirectory(patchZip, tempPath));
                StatusText.Text = $"Deleting US_PATCH_4.zip..."; await Task.Run(() => File.Delete(patchZip));
                await Task.Delay(500);
                ...
                await Task.Delay(500);
                StatusText.Text = $"Starting US_PATCH_4.exe..."; Process.Start(patchExe);
                await Task.Delay(300);
                bool patchRunning;
                StatusText.Text = $"Waiting for US_PATCH_4.exe...";
                if (Process.GetProcessesByName("US_PATCH_4").Length > 0)
                {
                    patchRunning = true;
                }
                else
                ...
                MessageBox.Show($"NFSPatcher has detected that US_PATCH_4.exe has closed.\n\nIf this is correct, please press \"Ok\".\n\nIf this is not correct, please wait until it exits and press\"Ok\".", "NFSPatcher", MessageBoxButton.OK, MessageBoxImage.Information);
                StatusText.Text = $"Deleting US_PATCH_4.exe..."; await Task.Run(() => File.Delete(patchExe));
                await Task.Delay(300);
